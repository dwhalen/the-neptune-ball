...
\NEW{Sign}{\sMantaPaper}{
  ...
  \s\MYloc	{- 002}
  \s\MYtext	{The papers go on for quite a while about some \cMother{\mer} named \cMother{} for some time. You can't really understand why though. The documents are clearly incomplete.
  }
  \s\MYitems {}
}
...
\NEW{Sign}{\sSpyPapers}{
  ...
  \s\MYloc	{- 002}
  \s\MYtext	{\cJade{} was a \pPacifica{}n \cJade{\prince} many years ago. There is very little information about \cJade{\them}
...
\NEW{Sign}{\sNorthStoneCover}{
  ...
  \s\MYloc	{-002}
  \s\MYtext	{\emph{Read the sign for ''An ornate chest'' before attempting to interact with this sign.} \\ The words on the large stone direct you to a book on a nearby bookshelf that has suddenly started to glow. You open the book to a confusing set of shifting numbers. \\ If you would like to attempt to solve this riddle, lift only \emph{this} sign to find the sign marked ``North Stone Question'' and find the riddle below.}
  \s\MYitems {}
 }
...
\NEW{Sign}{\sNorthStoneQuestion}{
  ...
  \s\MYloc	{-002}
  \s\MYtext	{Take a Sudoku puzzle from the envelope below and solve for the spaces marked with a red circle. Once you find the three numbers indicated, put them in numerical order and check your answer on the sign below this one. \\ \\
	{\bf Sudoku rules} \\
	The objective of a sudoku is to fill a 9Ã—9 grid with digits so that each column, each row, and each of the nine 3Ã—3 sub-grids that compose the grid contains all of the digits from 1 to 9 once. Each puzzle has a unique solution. You will not need to solve the entire Sudoku to complete this puzzle.}
  \s\MYitems {}
 }
