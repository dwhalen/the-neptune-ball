...
Gone. Or maybe never there? You must be getting closer though. You must be. Okay, deep breath. Maybe the music is just too faint to hear right now. You'll need to build something to amplify the sound. Find an \iConch{}, a \iPaper{} and a \iClam{}. Put them next to each other in alphabetical order by name (ignore articles like ``a'' and ``the'') to build the amplifying device. The second digit of each item number will lead you to the room where the music box is. Go to that room and search for ``\sPacketC{}''. If the music box is there, turn to page \nbref{eleventh}, if it is not there, turn to page \nbref{ninth}.
