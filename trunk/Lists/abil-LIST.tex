...
\NEW{Abil}{\aFirstAid}{
  ...
  \s\MYeffect	{You are no longer wounded or bloody. Take off you red headband immediately nd wake up after 2 minutes.}
}
...
\NEW{Abil}{\aRemoveDisguise}{
  ...
  \s\MYtext	{When you use this ability, put on a purple headband. You reveal your true self, \cQueen{\King} \cQueen{}. Open your ``R Envelope''.}
  \s\MYeffect	{I tear off my mask, alter my posture, and suddenly look totally different.}
  %\s\MYmems {\mREnvelope{}}
}

\NEW{Abil}{\aSummonGuards}{
  \s\MYname	{Summon Guards}
  \s\MYtext	{You summon the Royal Guards to bring one other PC to the Royal Dungeons. You must walk with the guards, as they escort your target into the dungeons. There are a maximum of 3 people in the dungeon at any one time.}
  \s\MYeffect	{Guards appear and roughly grab your arms. They assist me in escorting you to the dungeon. You are restrained as we walk.}
}

\NEW{Abil}{\aMonsterTransformation}{
  \s\MYname	{MonsterTransformation}
  \s\MYtext	{When you use this ability, call a game halt and find a GM. You resist any darkwater attack currently being levelled at you, break any restrains, and turn into a 10 foot tall terror of the deep. See the associated greensheet.}
...
\NEW{Abil}{\aInspiration}{
  ...
  \s\MYtext	{Use only when solving a trivia question or puzzle (as part of a riddle chain). Go find a GM and ask them the answer to one of the questions. You may use this ability twice in game at least 1 minute apart. If in doubt as to whether a question qualifies, ask a GM.}
  \s\MYeffect	{I know, I know!}
}

\NEW{Abil}{\aShark}{
  \s\MYname	{Collateral Damage}
  \s\MYtext	{Use only on a target without a red headband that you just knocked out. They are knocked out for 2 minutes instead of 5.}
  \s\MYeffect	{I knock you out fo 2 minutes instead of 5.}
}
