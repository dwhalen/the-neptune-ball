...
\NEW{MemPage}{\mExplosionPageTwo}{
  ...
  \s\MYtext	{increase your $\alpha$ score to 2, then go mute for 10 minutes.}
  }
...
\NEW{MemPage}{\mExplosionPageFour}{
  ...
  \s\MYtext	{A group of friendly spirits that only you can see appear out of thin air and smile down at you. You can use them to heal any wounded or unconscious person or to break any attack based restraint with a 30-count, after which they are healthy, conscious, and unrestrained. You can use this ability on yourself or someone else even if you are restrained or wounded yourself.}
}
...
\NEW{MemPage}{\mExplosionPageSix}{
  ...
  \s\MYtext	{Bolts of purple fire stream from your eyes and perform a CR 5 wound attack on the first two people you see (unassistable). If you can't see two people, run around until you do. Tell people out of game that this is clearly beyond your control. These attacks do not bloody you.
...
\NEW{MemPage}{\mSpyTwo}{
  \s\MYname	{How powerful is the Atlantican military? (Ask Gladius)}
...
\NEW{MemPage}{\mSpyThree}{
  \s\MYname	{How unified is Atlantica politically? (Ask King Triton or Manta)}
...
\NEW{MemPage}{\mSpyEight}{
  \s\MYname	{What is the current state of Pacifica's military? (Ask Jared or Valerie)}
...
\NEW{MemPage}{\mSpyNine}{
  \s\MYname	{How likely is Europa to get involved in an undersea war? (Ask Ariel)}
...
\NEW{MemPage}{\mSpyTen}{  %%Needs context
  \s\MYname	{How would Atlantica react to a draft? (Ask Gladius)}
...
\NEW{MemPage}{\mSpyEleven}{  %%Needs context
  \s\MYname	{How strong is the Assasin's Guild militarily? (Ask a member of the Assassin's Guild)}
...
\NEW{MemPage}{\mSpy}{
  ...
  \s\MYtext	{Beware the innocent and the seemingly innocent. One travels unseen with more skill than even an Assassin, and the other harbors a powerful hunger.}
}
...
\NEW{MemPage}{\mPlantB}{
  ...
  \s\MYtext	{This is the \cKing{\King}'s \cPlant{\sibling}. \cPlant{\They} is paranoid but harmless. \cPlant{\They} is not your suspect.}
}

\NEW{MemPage}{\mKingB}{
  \s\MYname	{Open if you do a background check on badge 531.}
  \s\MYtext	{This is the \cKing{\King}. \cKing{} has no secrets to hide. \cKing{\They} is not your suspect.}
  }

\NEW{MemPage}{\mQueenB}{
  \s\MYname	{Open if you do a background check on badge 580.}
  \s\MYtext	{\cQueen{}'s history is incomplete. No records of \cQueen{\them} extend more than 21 years back. This is unsurprising as this is when \cQueen{\they} fled the capital. On the other hand, \cAthena{} has no history beyond 34 years ago, despite being considerably older than that\ldots{} \cQueen{\They} is not your suspect.}
  }

\NEW{MemPage}{\mSlaveB}{
  \s\MYname	{Open if you do a background check on badge 649.}
  \s\MYtext	{This old \cSlave{\mer} is part of the Merchant's Guild. Oddly enough, \cSlave{\They} has no records more than 10 years back\ldots{}. \cSlave{\They} is not your suspect though.}
}

\NEW{MemPage}{\mWitchB}{
  \s\MYname	{Open if you do a background check on badge 237.}
  \s\MYtext	{This young \cWitch{\mer} has no records more than 10 years back\ldots{}. \cWitch{\They} is not your suspect though.}
}

\NEW{MemPage}{\mPriestB}{
  \s\MYname	{Open if you do a background check on badge 312.}
  \s\MYtext	{This \cPriest{\mer} seems to be connected to the the growing black market for children. Something smells fishy here. {\bf \cPriest{\They} is your suspect.} You may open page 2 of your research notebook entitled ``\nMafia{\MYname}''.}
}

\NEW{MemPage}{\mPrincessB}{
  \s\MYname	{Open if you do a backgrond check on badge 270.}
  \s\MYtext	{This is the \cPrincess{\Prince}. \cPrincess{\They} has nothing to hide and is not your suspect.}
}

\NEW{MemPage}{\mArielB}{
  \s\MYname	{Open if you do a background check on badge 386.}
  \s\MYtext	{\cAriel{} is now \cAriel{\King} of \pAmerica{}. \cAriel{\They} can't be involved with the mafia and is not your suspect.}
}

\NEW{MemPage}{\mWillowB}{
  \s\MYname	{Open if you do a background check on badge 143.}
  \s\MYtext	{A kid? A \emph{human} kid? Really? \cWillow{\they}'s not your suspect.}
}

\NEW{MemPage}{\mMantaB}{
  \s\MYname	{Open if you do a background check on badge 615.}
  \s\MYtext	{\cManta{} has a very interesting history, with many\ldots{} suspicious\ldots{} gaps. Whatever \cManta{\they} is up to, you should definitely keep an eye on \cManta{\them}. \cManta{\They} is not your suspect though.}
}

\NEW{MemPage}{\mDiplomatB}{
  \s\MYname	{Open if you do a background check on badge 276.}
  \s\MYtext	{As a \pPacifica{}n, your information on \cDiplomat{} is limited. But from what you can tell, \cDiplomat{\they} seems to appear out of nowhere about 3 years ago. \cDiplomat{\They} is not your suspect though.}
}

\NEW{MemPage}{\mBodyguardB}{
  \s\MYname	{Open if you do a background check on badge 397.}
  \s\MYtext	{As a \pPacifica{}n, your information on \cBodyguard{} is limited. Records do indicate that \cBodyguard{} seems to have gone to great lengths to have a child, but \cBodyguard{\they} is not your suspect.}
}

\NEW{MemPage}{\mPrinceB}{
  \s\MYname	{Open if you do a background check on badge 271.}
  \s\MYtext	{As a \pPacifica{}n, your information on \cPrince{} is limited, but \cPrince{} is the \cPrince{\Prince} of \pPacifica{}, without anything to hide. \cPrince{\They} is not your suspect.}
}

\NEW{MemPage}{\mSpyB}{
  \s\MYname	{Open if you do a background check on badge 347.}
  \s\MYtext	{As a \pPacifica{}n, your information on \cSpy{} is limited, but \cSpy{\they} appears to have no record beyond 17 years ago. \cSpy{\They} is not your suspect though.}
}
...
\NEW{MemPage}{\mDepressionPageTwo}{
  \s\MYname	{Open if either Ash or Ariel expresses a desire to stay (positive).}
...
\NEW{MemFold}{\mWPacket}{
  \s\MYname	{W packet}
  \s\MYtext	{\cSpy{} is clearly researching the history of the war -- your past. \cSpy{\They} might help you prove that \cExExKing{} kidnapped and raped your \cMother{\parent}, leading to your conception. But at the same time, you can't fully trust \cSpy{\them}. Mentioning \cMother{} should be enough to set \cSpy{} on th right track without giving too much away.}
}
...
\NEW{MemFold}{\mQPacketManta}{
  \s\MYname	{Open if you see a purple headband}
  \s\MYtext	{Drat, \cQueen{\they}'s revealed \cQueen{\their}self. This will make it much harder to get rid of \cQueen{\their}...}
 }
...
\NEW{MemFold}{\mQPacketGladius}{
  \s\MYname	{Open if you see a purple headband}
  \s\MYtext	{Oh shit. \cQueen{} has revealed herself as \cAthena{} - you were afraid this was going to happen. \cKing{} is going to be pissed, and you may very well lose her...}
 }
 
\NEW{MemEnvelope}{\mREnvelope}{
  \s\MYname	{R Envelope}
  \s\MYtext	{As a member of the royal family, you have the power to summon the guards to your aid.}
  \s\MYabils	{\aSummonGuards{}}
}

\NEW{MemEnvelope}{\mMEnvelope}{
  \s\MYname	{M Envelope}
  \s\MYtext	{The water turns murky before your very eyes. Dark voices whisper terrible things in your ears. A great pressure builds on your chest and you find it hard to breath. You've been tricked by dark sorcery! \emph{Go ``Not Here'' and read the two greensheets in this packet. Return to game as soon as you are ready.}}
  \s\MYgreens	{\gPotions{} \gSpirits{}}
  \s\MYabils{\aLesserDispel{}}
}
