...
\NEW{MemPage}{\mDepressionPageOne}{
  \s\MYname	{Open at \cTThreeThirty{} if your $/Beta$ is 6 or higher and no treaty has reached the Complete state this game (negative).}
  \s\MYtext	{You're never going to get the treaty signed at this rate! Decrease your $/Beta$ by 2.}
}
	
\NEW{MemPage}{\mDepressionPageTwo}{
  \s\MYname	{Open if either \cWillow{} or \cAriel{} expresses a desire to stay (positive).}
...
\NEW{MemPage}{\mDepressionPageFour}{
  ...
  \s\MYtext	{How embarassing... Decrease your $/Beta$ by 2.}
}
...
\NEW{MemFold}{\mQPacketGeneric}{
  \s\MYname	{Open if you see a purple headband}
  \s\MYtext	{Holy shit! That's \cQueen{\King} \cQueen{\MYname}! \cQueen{\They}'s been missing for 21 years!}
 }
