...
\PRESETS{Char}{
  ...
  \newstat\MYomega	{$\gamma$}{$\gamma$}{0} % Underground connections
  \newstat\MYbeta	{$\beta$}{$\beta$}{0} % Emotional stats
  %\newstat\MYalpha	{$\alpha$}{$\alpha$}{0} % Emotional stats
...
\NEW{PC}{\cWitch}{  %%Ariel.tex, Manta.tex and EricsBrother.tex know her by Rumplstiltskin (her old name) via a nickname macro
  ...
  \s\MYgreens	{\gDecking{} \gLove{} \gPotions{}}
  \s\MYabils	{\aPerceive{} \aLesserDispel{} \aGreaterDispel{} \aMonsterTransformation{}}
  \s\MYitems	{\iPearl{} \iAmulet{}}
  \s\MYwhites	{}
  \s\MYmems {}
...
\NEW{PC}{\cSlave}{ %% Eric and Ariel know him by the name Joreth (he is in disguise right now)
   \s\MYname	{Morvyn}
   \s\MYfile	{Slave.tex}
   \s\MYsex      {\male}
   \s\MYnumber	{649}
...
\NEW{PC}{\cWillow}{
  ...
  \s\MYblues	{}
  \s\MYgreens	{}
  \s\MYabils	{\aInspiration{} \aRunAway{} \aUnnoticed{}}
  \s\MYitems	{}
  \s\MYwhites	{}
  \s\MYmems {\mExplosion{}}
