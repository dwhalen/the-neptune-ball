...
\NEW{PC}{\cWitch}{  %%Ariel.tex, Manta.tex and EricsBrother.tex know her by Rumplstiltskin (her old name) via a nickname macro
   \s\MYname	{Ursula}
   \s\MYfile	{SeaWitch.tex}
   \s\MYsex      {\female}
   \s\MYnumber	{}
   \s\MYdesc     {A stunningly beautiful merwoman}   
...
\NEW{PC}{\cManta}{
   \s\MYname	{Morvyn Nakamura} %%Derivative of Morgan. Nod to Morgan/Mordred from the Arthur story. (Mordred also means "sea council")
   \s\MYfile	{Manta.tex}
   \s\MYsex      {\male}
   \s\MYnumber	{}
   \s\MYdesc     {}   
...
\NEW{PC}{\cDaughter}{ 
   \s\MYname	{Oshyn} %%Derivative of Oshun, african goddess of love, lust and diplomacy. Also an English name with 
   %%oblique reference to the ocean.
   \s\MYfile	{YoungestDaughter.tex}
   \s\MYsex      {\female}
   \s\MYnumber	{}
   \s\MYdesc     {}   
...
\NEW{PC}{\cAthena}{ 
   \s\MYname	{Minerva} %%Roman version of Athena
   \s\MYfile	{Athena.tex}
   \s\MYsex      {\female}
   \s\MYnumber	{}
   \s\MYdesc     {}   
...
\NEW{PC}{\cPrince}{ %% Eric and Ariel know him by the name Joreth (he is in disguise right now)
   \s\MYname	{Eric's Brother}
   \s\MYfile	{EricsBrother.tex}
   \s\MYsex      {\male}
   \s\MYnumber	{}
   \s\MYdesc     {}   
...
\NEW{PC}{\cBodyguard}{
   \s\MYname	{Valerie}
   \s\MYfile	{PacificanDeserter.tex}
   \s\MYsex      {\female}
   \s\MYnumber	{}
   \s\MYdesc     {}   
...
\NEW{Name}{\cVHusband}{ %%In case we need tochange the name later (also easy to add  dif name to pacificans)
	\s\MYname   {John}
	\s\MYsex			{\male}
}
